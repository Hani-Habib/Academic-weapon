...
class ConsumerClass:
    ...
    def utility(self,x1,x2,x3):
        ...
        # nest: bus and train into a travel composite
        xB = self.ces(x2,x3,par.beta,par.sigma_B)

        # food and travel into utility
    ...
    def solve_grid(self,N=200,do_print=True):
        ...
        if do_print:
            print(f'grid search with ({N*N} evaluations)')
            print(f'  s1 = {opt.s1:.6f}, w = {opt.w:.6f}')
            print(f'  shares: {opt.s1:.6f}, {opt.s2:.6f}, {opt.s3:.6f}'
                  f' (sum = {opt.s1+opt.s2+opt.s3:.6f})')
            print(f'  u = {opt.u:.6f}')
    ...
    def solve(self,s0=None,do_print=True,**kwargs):
        ...
        res = optimize.minimize(self.objective,s0,method='L-BFGS-B',
            bounds=((0,1),(0,1)),
            callback=lambda sk: path.append(sk.copy()),**kwargs)
        ...
        if do_print:
            print(f'L-BFGS-B from s0 = ({s0[0]:.2f},{s0[1]:.2f})')
            print(f'  s1 = {opt.s1:.6f}, w = {opt.w:.6f}')
            print(f'  shares: {opt.s1:.6f}, {opt.s2:.6f}, {opt.s3:.6f}')
            print(f'  u = {opt.u:.6f}')
            print(f'  {res.nit} iterations, {res.nfev} evaluations of u,'
